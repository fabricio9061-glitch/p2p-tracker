document.addEventListener('DOMContentLoaded',()=>{
    ...
    const _bCerrarSync=$('btnCerrarSyncDiag');if(_bCerrarSync)_bCerrarSync.addEventListener('click',()=>cerrarModal('modalSyncDiag'));
    const _bRefSync=$('btnSyncDiagRefresh');if(_bRefSync)_bRefSync.addEventListener('click',renderDiagnosticoSync);
    const _bForceSync=$('btnSyncDiagForceSync');if(_bForceSync)_bForceSync.addEventListener('click',ejecutarForzarSync);
    const _bCopySync=$('btnSyncDiagCopy');if(_bCopySync)_bCopySync.addEventListener('click',copiarDiagnosticoSync);
    const _bAuditSync=$('btnSyncDiagAudit');if(_bAuditSync)_bAuditSync.addEventListener('click',()=>{
        if(_bAuditSync){_bAuditSync.disabled=true;_bAuditSync.textContent='Analizando…'}
        /* setTimeout para que el botón pueda actualizar UI antes del stringify pesado */
        setTimeout(()=>{
            try{
                window._ultimaAuditoria=analizarPayload(AppState.datos);
                _syncLog('audit:done',{totalKB:window._ultimaAuditoria&&window._ultimaAuditoria.totalKB,
                                       estado:window._ultimaAuditoria&&window._ultimaAuditoria.estado});
                renderDiagnosticoSync();
            }catch(e){
                console.error('[P2P] Análisis falló:',e);
                alert('Análisis falló: '+e.message);
            }finally{
                if(_bAuditSync){_bAuditSync.disabled=false;_bAuditSync.textContent='🔍 Auditar tamaño'}
            }
        },50);
    });
    const _bAuditCp=$('btnSyncDiagAuditCp');if(_bAuditCp)_bAuditCp.addEventListener('click',()=>{
        if(_bAuditCp){_bAuditCp.disabled=true;_bAuditCp.textContent='Verificando…'}
        /* setTimeout para que el botón pueda actualizar UI antes del análisis */
        setTimeout(()=>{
            try{
                window._ultimaAuditoriaComision=auditarComisionPlataforma();
                _syncLog('audit-comision:done',{
                    verdict:window._ultimaAuditoriaComision.verdict,
                    exact:window._ultimaAuditoriaComision.exact,
                    tolerance:window._ultimaAuditoriaComision.tolerance,
                    small:window._ultimaAuditoriaComision.small,
                    significant:window._ultimaAuditoriaComision.significant,
                    missingPct:window._ultimaAuditoriaComision.missingPct
                });
                renderDiagnosticoSync();
            }catch(e){
                console.error('[P2P] Auditoría comisión falló:',e);
                alert('Auditoría comisión falló: '+e.message);
            }finally{
                if(_bAuditCp){_bAuditCp.disabled=false;_bAuditCp.textContent='💰 Auditar comisión'}
            }
        },50);
    });
    /* v4.7.59 — botón Auditar splits para detectar ops corruptas */
    const _bAuditSp=$('btnSyncDiagAuditSplits');if(_bAuditSp)_bAuditSp.addEventListener('click',()=>{
        if(_bAuditSp){_bAuditSp.disabled=true;_bAuditSp.textContent='Verificando…'}
        setTimeout(()=>{
            try{
                const r=auditarSplitsCorruptos();
                window._ultimaAuditoriaSplits=r;
                let detalle=r.resumen+'\n\n';
                if(r.bancoVacio.length){
                    detalle+=`━ Banco vacío (${r.bancoVacio.length}) ━\n`;
                    r.bancoVacio.slice(0,5).forEach(o=>{
                        detalle+=`  ${o.fecha} ${o.hora} · ${o.tipo} · $${fmtNum(o.monto,2)} · banco principal: ${o.banco}\n`;
                    });
                    if(r.bancoVacio.length>5)detalle+=`  ... y ${r.bancoVacio.length-5} más\n`;
                    detalle+='\n';
                }
                if(r.bancoInexistente.length){
                    detalle+=`━ Banco inexistente (${r.bancoInexistente.length}) ━\n`;
                    r.bancoInexistente.slice(0,5).forEach(o=>{
                        detalle+=`  ${o.fecha} ${o.hora} · ${o.tipo} · $${fmtNum(o.monto,2)} · fantasmas: ${o.bancosFantasma.join(', ')}\n`;
                    });
                    if(r.bancoInexistente.length>5)detalle+=`  ... y ${r.bancoInexistente.length-5} más\n`;
                    detalle+='\n';
                }
                if(r.sumaIncorrecta.length){
                    detalle+=`━ Suma incorrecta (${r.sumaIncorrecta.length}) ━\n`;
                    r.sumaIncorrecta.slice(0,5).forEach(o=>{
                        detalle+=`  ${o.fecha} ${o.hora} · esperado $${fmtNum(o.esperado,2)} · suma $${fmtNum(o.sumaAportes,2)} · diff $${fmtNum(o.diferencia,2)}\n`;
                    });
                    if(r.sumaIncorrecta.length>5)detalle+=`  ... y ${r.sumaIncorrecta.length-5} más\n`;
                    detalle+='\n';
                }
                detalle+='Esta es una herramienta de diagnóstico — no se modificó ningún dato.\nRevisar y editar/borrar manualmente las operaciones afectadas si es necesario.';
                alert(detalle);
                renderDiagnosticoSync();
            }catch(e){
                console.error('[P2P] Auditoría splits falló:',e);
                alert('Auditoría splits falló: '+e.message);
            }finally{
                if(_bAuditSp){_bAuditSp.disabled=false;_bAuditSp.textContent='🧩 Auditar splits'}
            }
        },50);
    });
    /* v4.7.63 — botón Auditar compresión wire */
    const _bAuditWire=$('btnSyncDiagAuditWire');if(_bAuditWire)_bAuditWire.addEventListener('click',()=>{
        if(_bAuditWire){_bAuditWire.disabled=true;_bAuditWire.textContent='Verificando…'}
        setTimeout(()=>{
            try{
                const r=auditarCompresionWire();
                window._ultimaAuditoriaCompresion=r;
                let detalle='━━━ AUDITORÍA COMPRESIÓN WIRE ━━━\n\n';
                detalle+='Veredicto: '+r.verdict+'\n';
                detalle+='Total ops: '+r.totalOps+'\n';
                detalle+='Round-trip OK: '+r.roundtripOk+'/'+r.totalOps+'\n';
                detalle+='\nTamaños:\n';
                detalle+='  Sin comprimir: '+Math.round(r.sizeBefore/1024)+' KB\n';
                detalle+='  Comprimido:    '+Math.round(r.sizeAfter/1024)+' KB\n';
                detalle+='  Ahorro:        '+r.savingsKB+' KB ('+(r.sizeBefore>0?Math.round(100*(r.sizeBefore-r.sizeAfter)/r.sizeBefore):0)+'%)\n';
                if(r.bancoNoMapeado.length){
                    detalle+='\n⚠ Bancos NO mapeados ('+r.bancoNoMapeado.length+'):\n';
                    r.bancoNoMapeado.slice(0,5).forEach(b=>detalle+='  • '+b.banco+' (op '+b.id+')\n');
                    if(r.bancoNoMapeado.length>5)detalle+='  ... y '+(r.bancoNoMapeado.length-5)+' más\n';
                }
                if(r.tipoNoMapeado.length){
                    detalle+='\n⚠ Tipos NO mapeados ('+r.tipoNoMapeado.length+'):\n';
                    r.tipoNoMapeado.slice(0,5).forEach(t=>detalle+='  • '+t.tipo+' (op '+t.id+')\n');
                }
                if(r.monedaNoMapeado.length){
                    detalle+='\n⚠ Monedas NO mapeadas ('+r.monedaNoMapeado.length+'):\n';
                    r.monedaNoMapeado.slice(0,5).forEach(m=>detalle+='  • '+m.moneda+' (op '+m.id+')\n');
                }
                if(r.opsConDiff.length){
                    detalle+='\n⚠ Round-trip con diff:\n';
                    r.opsConDiff.forEach(o=>{
                        detalle+='  • op '+o.id+': ';
                        detalle+=o.diffs.map(d=>d.campo+' '+JSON.stringify(d.antes)+'→'+JSON.stringify(d.despues)).join(', ');
                        detalle+='\n';
                    });
                }
                detalle+='\nHerramienta de diagnóstico — no modifica datos.';
                alert(detalle);
                renderDiagnosticoSync();
            }catch(e){
                console.error('[P2P] Auditoría compresión falló:',e);
                alert('Auditoría compresión falló: '+e.message);
            }finally{
                if(_bAuditWire){_bAuditWire.disabled=false;_bAuditWire.textContent='🗜️ Auditar compresión'}
            }
        },50);
    });
    /* v4.7.64 — botón manual Recovery write */
    const _bRecovery=$('btnSyncDiagRecovery');if(_bRecovery)_bRecovery.addEventListener('click',()=>{
        const msg=
            '🚨 RECOVERY WRITE\n\n'+
            'Esto va a forzar la migración del documento remoto al formato comprimido.\n\n'+
            '• Bloquea toda la app durante la operación (~30-60s).\n'+
            '• No vas a poder operar hasta que termine.\n'+
            '• Si falla, entra en modo seguro con opciones manuales.\n\n'+
            '¿Continuar?';
        if(!confirm(msg))return;
        cerrarModal('modalSyncDiag');
        setTimeout(()=>{
            if(typeof window.recoveryWrite==='function'){
                window.recoveryWrite({trigger:'manual-diag'});
            }else{
                alert('Recovery no disponible (función no encontrada).');
            }
        },300);
    });
