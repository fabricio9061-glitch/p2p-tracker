...
function abrirDiagnosticoSync(){
    const modal=$('modalSyncDiag');if(!modal)return;
    /* Antes de mostrar: sanear estados huérfanos + reevaluar errores residuales.
       Esto evita que el panel muestre síntomas de bug que ya están resueltos. */
    try{
        if(typeof repairOrphanPendingStates==='function')repairOrphanPendingStates();
        if(typeof reevaluarEstadoSync==='function')reevaluarEstadoSync();
    }catch(e){console.warn('[P2P] saneamiento pre-diagnóstico falló:',e.message)}
    renderDiagnosticoSync();
    modal.classList.add('active');
}
function renderDiagnosticoSync(){
    const el=$('syncDiagContent');if(!el)return;
    const d=getSyncDiagnostics();
    const fmtBool=b=>b?'✓':'✗';
    const fmtTime=t=>t?new Date(t).toLocaleTimeString():'—';
    let html='';
    html+='━━━ ESTADO GENERAL ━━━\n';
    html+='Versión app:      '+d.version+'\n';
    html+='Online:           '+fmtBool(d.online)+'\n';
    html+='Estado sync:      '+d.syncState+'\n';
    html+='Listener activo:  '+fmtBool(d.listenerActive)+'\n';
    html+='Recovery activa:  '+fmtBool(d.recoveryActive)+'\n';
    /* v4.7.42: estado del cliente Firebase. Si está terminado, sync remoto bloqueado. */
    if(d.clientTerminated){
        html+='Firebase client:  🛑 TERMINATED — requiere recarga\n';
        if(d.clientTerminatedAt)html+='Terminó a las:    '+new Date(d.clientTerminatedAt).toLocaleTimeString()+'\n';
    }else{
        html+='Firebase client:  ✓ activo\n';
    }
    html+='\n━━━ VERSIONES ━━━\n';
    html+='_localVersion:    '+d.localVersion+'\n';
    html+='_datos._version:  '+d.dataVersion+'\n';
    html+='Match:            '+fmtBool(d.localVersion===d.dataVersion)+'\n';
    if(d.restoredFrom)html+='Restored from:    '+d.restoredFrom+'\n';
    if(d.postRestoreLockExpires)html+='Lock expira:      '+d.postRestoreLockExpires+'\n';
    if(d.datosStale)html+='⚠ Datos stale (esperando reconciliación)\n';
    html+='\n━━━ QUEUE & FLAGS ━━━\n';
    html+='_syncQueue size:  '+d.syncQueueSize+'\n';
    html+='_localDirty:      '+d.localDirty+'\n';
    html+='_guardando:       '+fmtBool(d.guardando)+'\n';
    html+='_guardarPendiente:'+fmtBool(d.guardarPendiente)+'\n';
    html+='_syncPending:     '+d.syncPending+'\n';
    html+='_syncErrors:      '+d.syncErrors+'\n';
    html+='_retryDelay:      '+(d.retryDelay/1000).toFixed(1)+'s\n';
    html+='Retry pendiente:  '+fmtBool(d.retryActive)+'\n';
    html+='Debounce activo:  '+fmtBool(d.debounceActive)+'\n';
    if(d.syncQueueSample&&d.syncQueueSample.length){
        html+='\nPrimeros pending:\n';
        d.syncQueueSample.forEach(s=>{
            /* safeIdTail evita el bug "(s.id||'').slice is not a function" cuando uid 
               devuelve Number. Adicionalmente logeamos typeof para forensia: si vemos
               typeof inesperado, hay un path de creación de IDs corrupto. */
            html+='  '+s.type+' '+s.entity+' '+safeIdTail(s.id,8)+' ['+typeof s.id+'] ('+fmtTime(s.ts)+')\n';
        });
    }
    html+='\n━━━ LOG RECIENTE (últimos 30) ━━━\n';
    if(d.logTail&&d.logTail.length){
        d.logTail.forEach(e=>{
            const t=new Date(e.ts).toLocaleTimeString();
            const {ts,event,...rest}=e;
            const det=Object.keys(rest).length?' '+JSON.stringify(rest):'';
            html+=t+' '+event+det+'\n';
        });
        /* ═══ Análisis de timing del último save completo ═══
           Buscamos hacia atrás el patrón save:start ... save:confirmed
           más reciente y reportamos métricas. */
        const tail=d.logTail;
        let lastConfirmed=-1,matchedStart=-1;
        for(let i=tail.length-1;i>=0;i--){
            if(tail[i].event==='save:confirmed'){lastConfirmed=i;break}
        }
        if(lastConfirmed>=0){
            for(let i=lastConfirmed-1;i>=0;i--){
                if(tail[i].event==='save:start'){matchedStart=i;break}
            }
        }
        if(lastConfirmed>=0&&matchedStart>=0){
            const totalMs=new Date(tail[lastConfirmed].ts).getTime()-new Date(tail[matchedStart].ts).getTime();
            const writeEvt=tail.find((e,i)=>i>matchedStart&&i<=lastConfirmed&&e.event==='save:write-end');
            html+='\n━━━ TIMING DEL ÚLTIMO SAVE ━━━\n';
            html+='Total (start→confirm): '+totalMs+' ms\n';
            if(writeEvt&&writeEvt.totalMs)html+='Tiempo de write:       '+writeEvt.totalMs+' ms\n';
            if(writeEvt&&writeEvt.ms)html+='Solo set() en server:  '+writeEvt.ms+' ms\n';
            const prepEvt=tail.find((e,i)=>i>matchedStart&&i<=lastConfirmed&&e.event==='save:prep-end');
            if(prepEvt){
                if(prepEvt.payloadKB)html+='Payload enviado:       '+prepEvt.payloadKB+' KB\n';
                if(prepEvt.baselineKB&&prepEvt.baselineKB>prepEvt.payloadKB){
                    /* Mostrar la historia completa: baseline → preStrip → final */
                    html+='Baseline (sin strip):  '+prepEvt.baselineKB+' KB\n';
                    const savedPerOp=prepEvt.baselineKB-(prepEvt.preKB||prepEvt.payloadKB);
                    if(savedPerOp>0){
                        html+='Ahorro per-op:         '+savedPerOp+' KB (consumedLots, ganancia)\n';
                    }
                    if(prepEvt.preKB&&prepEvt.preKB!==prepEvt.payloadKB){
                        html+='Ahorro top-level:      '+(prepEvt.preKB-prepEvt.payloadKB)+' KB (lotes, saldoUsdt)\n';
                    }
                    html+='Reducción TOTAL:       '+(prepEvt.reductionTotalKB||0)+' KB ('+(prepEvt.reductionTotalPct||0)+'%)\n';
                    if(Array.isArray(prepEvt.stripped))html+='Campos excluidos:      '+prepEvt.stripped.join(', ')+'\n';
                }else if(prepEvt.preKB&&prepEvt.preKB!==prepEvt.payloadKB){
                    /* Fallback: solo tenemos preStrip vs final (no baseline) */
                    html+='Antes del strip:       '+prepEvt.preKB+' KB\n';
                    html+='Reducción:             '+(prepEvt.reductionPct||0)+'% ('+(prepEvt.preKB-prepEvt.payloadKB)+' KB)\n';
                    if(Array.isArray(prepEvt.stripped))html+='Campos excluidos:      '+prepEvt.stripped.join(', ')+'\n';
                }
                if(prepEvt.ms)html+='Prep payload:          '+prepEvt.ms+' ms\n';
            }
            /* Detectar rollback falso seguido de confirmación posterior */
            const rollbackEvt=tail.find((e,i)=>i>matchedStart&&i<=lastConfirmed&&e.event==='save:version-rollback');
            if(rollbackEvt){
                html+='⚠ Rollback detectado:   '+(rollbackEvt.reason||'?')+' (toVersion: '+rollbackEvt.toVersion+')\n';
                html+='  Pero confirmación llegó después → write OK pese al rollback\n';
            }
        }
    }else{
        html+='(sin eventos registrados aún)\n';
    }
    /* ═══ Sección de auditoría de payload (bajo demanda) ═══
       Si _ultimaAuditoria está cacheada (del botón "Auditar tamaño"), mostrarla.
       NO la calculamos en cada render — es cara (~50ms). */
    if(window._ultimaAuditoria){
        const a=window._ultimaAuditoria;
        html+='\n━━━ AUDITORÍA DE TAMAÑO DEL DOCUMENTO ━━━\n';
        html+='Estado (payload remoto): '+a.estado+'\n';
        /* v4.7.41: separar lo que va a Firestore de lo que vive solo en memoria */
        if(a.remoteKB){
            html+='Payload REMOTO (real):   '+a.remoteKB+' KB ← lo que sube a Firestore\n';
            html+='Estado LOCAL completo:   '+a.totalKB+' KB (incluye derivables)\n';
            html+='Ahorrado por strip:      '+a.savedByStripKB+' KB ('+a.savedByStripPct+'%)\n';
        }else{
            html+='Total KB:                '+a.totalKB+'\n';
        }
        html+='Cantidad ops:            '+a.cantidadOps+'\n';
        if(a.avgBytesPerOp)html+='Promedio por op:         '+a.avgBytesPerOp+' bytes\n';
        if(a.memUsedMB)html+='Heap JS usado:           '+a.memUsedMB+' MB / '+a.memLimitMB+' MB\n';
        html+='Tiempo stringify:        '+a.tiempoStringifyMs+' ms\n';
        html+='\nTop secciones por peso (state local):\n';
        a.topKeys.slice(0,10).forEach(k=>{
            html+='  '+k.key.padEnd(22)+(k.kb+' KB').padStart(8)+'  ('+k.pct+'%)';
            if(k.tipo==='array')html+=' ['+k.len+' items]';
            html+='\n';
        });
        if(a.campoOpsTop&&a.campoOpsTop.length){
            html+='\nTop campos DENTRO de operaciones (state local):\n';
            a.campoOpsTop.forEach(c=>{
                html+='  '+c.campo.padEnd(22)+(c.kbTotal+' KB').padStart(8)+'  ('+c.pct+'% de ops, '+c.freq+'/'+a.cantidadOps+')\n';
            });
        }
        if(a.topOperaciones&&a.topOperaciones.length){
            html+='\nTop 10 ops más pesadas:\n';
            a.topOperaciones.forEach(o=>{html+='  id '+o.id+': '+o.bytes+' bytes\n'});
        }
        if(a.valoresRepetidos&&a.valoresRepetidos.length){
            html+='\nValores repetidos (candidatos a normalizar):\n';
            a.valoresRepetidos.forEach(r=>{
                html+='  '+r.campo+'="'+r.valor+'" × '+r.count+' (~'+r.bytesGastadosUnnecesarios+' bytes acumulados)\n';
            });
        }
        if(a.flags){
            /* v4.7.41: solo mostrar flags cuando hay un problema real.
               Antes marcábamos "lotes embebidos" y "saldoUsdt persistido" como ⚠ pero 
               eran falsos positivos — el state local SIEMPRE tiene esos campos porque 
               recalcular los reconstruye. El payload remoto NO los lleva (verificable
               en remoteKB vs totalKB). Ahora solo flags cuando son problemas reales. */
            const flagLines=[];
            if(a.flags.tasasRecientesLen>10)flagLines.push('⚠ tasasRecientes muy largo: '+a.flags.tasasRecientesLen+' items');
            if(a.flags.dismissedVersionsLen>20)flagLines.push('⚠ dismissedVersions sin limpiar: '+a.flags.dismissedVersionsLen+' items');
            if(a.flags.syncStateOrphans>0)flagLines.push('⚠ _syncState orphans en datos: '+a.flags.syncStateOrphans);
            if(a.flags.tagsLen>50)flagLines.push('• tags: '+a.flags.tagsLen+' (acumulación)');
            if(flagLines.length>0){
                html+='\nFlags (problemas reales):\n';
                flagLines.forEach(l=>{html+='  '+l+'\n'});
            }else{
                html+='\nFlags: ✓ ningún problema detectado\n';
            }
        }
        html+='\n(análisis hecho en '+a.tiempoAnalisisMs+' ms)\n';
    }else{
        html+='\n━━━ AUDITORÍA DE TAMAÑO ━━━\n';
        html+='Tocá "🔍 Auditar tamaño" para analizar qué pesa en el documento.\n';
    }
    /* ═══ Auditoría de comisionPlataforma (bajo demanda) ═══ */
    if(window._ultimaAuditoriaComision){
        const c=window._ultimaAuditoriaComision;
        html+='\n━━━ AUDITORÍA DE comisionPlataforma ━━━\n';
        html+='Verdict: '+c.verdict+'\n';
        if(c.verdictReason)html+='Razón: '+c.verdictReason+'\n';
        html+='\nDistribución por categoría:\n';
        html+='  exact (sin diff):     '+c.exact+'\n';
        html+='  tolerance (≤0.01):    '+c.tolerance+'\n';
        html+='  small (0.01-0.10):    '+c.small+(c.small>0?' ⚠':'')+'\n';
        html+='  significant (>0.10):  '+c.significant+(c.significant>0?' 🛑':'')+'\n';
        html+='  legacy (sin Pct):     '+c.missingPct+(c.missingPct>0?' ⚠':'')+'\n';
        html+='  sin Plataforma:       '+c.missingCp+'\n';
        html+='  inválidas:            '+c.invalid+'\n';
        html+='  TOTAL ops:            '+c.totalOps+'\n';
        if(c.worstCases&&c.worstCases.length>0){
            html+='\nTop 10 discrepancias (mayor a menor):\n';
            c.worstCases.forEach(w=>{
                html+='  ['+w.category+'] id '+w.id+' fecha '+w.fecha+' '+w.tipo+'\n';
                html+='    guardada: '+w.cpGuardada+' / esperada: '+w.cpExpected+' / diff: '+w.diff.toFixed(4)+'\n';
                html+='    monto: '+w.monto+' tasa: '+w.tasa+' usdt: '+w.usdt+' pct: '+w.comisionPct+'\n';
            });
        }
        if(c.legacyCases&&c.legacyCases.length>0){
            html+='\nCasos legacy (sin comisionPct):\n';
            c.legacyCases.forEach(l=>{
                html+='  id '+l.id+' fecha '+l.fecha+' '+l.tipo+' cpGuardada: '+l.cpGuardada+'\n';
            });
        }
        html+='\nFórmula usada: '+c.formula+'\n';
        html+='(análisis hecho en '+c.tiempoAnalisisMs+' ms)\n';
    }
    el.textContent=html;
}
async function ejecutarForzarSync(){
    const btn=$('btnSyncDiagForceSync');if(btn){btn.disabled=true;btn.textContent='Sincronizando…'}
    try{
        const r=await forzarSyncManual();
        setTimeout(()=>{renderDiagnosticoSync();if(btn){btn.disabled=false;btn.textContent='⚡ Forzar sync'}},500);
        return r;
    }catch(e){
        if(btn){btn.disabled=false;btn.textContent='⚡ Forzar sync'}
    }
}
function copiarDiagnosticoSync(){
    const el=$('syncDiagContent');if(!el)return;
    try{
        navigator.clipboard.writeText(el.textContent).then(()=>{
            const b=$('btnSyncDiagCopy');if(b){const o=b.textContent;b.textContent='✓ Copiado';setTimeout(()=>b.textContent=o,1500)}
        });
