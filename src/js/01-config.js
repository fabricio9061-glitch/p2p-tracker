...
const CHANGELOG = [
    ...
    {version:'5.8.0', date:'2026-08-15', headline:'🔗 Todo se reconstruye desde los eventos: lotes, ganancias y ahora también saldos.', changes:[
        ...
        {type:'improve', title:'El efecto de cada evento vive en un solo lugar', desc:'Compras con pago dividido, ventas, ajustes de saldo, transferencias con comisión y conversiones entre monedas: cada uno tiene ahora una definición única de qué cuentas toca y por cuánto. Antes esa lógica estaba repetida en cada punto donde se creaba, editaba o borraba un registro, que es la razón por la que un caso podía quedar desalineado de los demás.'}
    ]},
    {version:'5.7.2', date:'2026-08-15', headline:'🛡️ Corregidos el saldo que no se descontaba y el USDT duplicado, con vigilancia permanente.', changes:[
        {type:'fix', title:'A veces una venta no descontaba el saldo', desc:'El saldo de cada cuenta no se recalcula: se le suma y se le resta, y ese resultado es el único registro que existe. Al cargar una operación se anotaba como pendiente la operación en sí, pero no el cambio de saldo, así que la protección que impide que el servidor pise cambios locales no lo veía. Si en ese instante llegaba una foto del servidor —el eco de un guardado anterior, o el otro dispositivo— el saldo remoto pisaba al recién descontado y la resta desaparecía para siempre, porque nadie la vuelve a calcular. Ahora el cambio de saldo se anota igual que la operación, y mientras esté sin subir manda lo local.'},
        {type:'fix', title:'A veces una compra duplicaba el USDT', desc:'Los lotes de arrastre que crea el archivado tienen una cantidad declarada, pero el motor fusiona dentro de ellos las compras hechas a la misma tasa, así que la cantidad que queda en pantalla es la declarada más lo absorbido. Cuando el documento del servidor llegaba sin ese campo, la app lo reconstruía leyendo esos lotes ya calculados y tomaba la cantidad inflada como nueva declaración: la compra pasaba a contarse dos veces, dentro del lote de arrastre y como operación propia. Ahora esa reconstrucción se hace una sola vez al cargar, leyendo lo guardado y nunca el resultado del cálculo.'},
        {type:'new', title:'Auditoría automática de saldos', desc:'El saldo USDT se recalcula desde cero en cada actualización, así que un error se corrige solo. El saldo de las cuentas no: se muta, y un error queda ahí para siempre arrastrándose sobre todo lo que sigue. Esa diferencia es la que produjo la falla anterior. Ahora, cada vez que la app cambia un saldo a propósito anota cuál debería ser el resultado, y en el siguiente dibujado compara. Si un saldo cambió sin una acción que lo explique, queda registrado con el detalle y aparece un aviso, una sola vez para no interrumpir. No corrige nada por su cuenta: un ajuste automático sobre dinero, a partir de un diagnóstico que puede estar equivocado, sería peor que el problema. El historial completo se consulta con auditoriaSaldos() en la consola.'}
    ]},
];
