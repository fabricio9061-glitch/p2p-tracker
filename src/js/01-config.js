 ...
 */
const CHANGELOG = [
    {version:'6.0.0', date:'2026-08-16', headline:'🧮 Los saldos ahora se llevan como los lleva un contador.', changes:[
        {type:'improve', title:'El saldo de cada cuenta es el resultado de tus operaciones', desc:'Un contador no guarda el saldo de una cuenta: lo obtiene sumando los asientos, y si algo no cuadra vuelve a sumar. La app hacía lo contrario con las cuentas bancarias: guardaba el saldo y lo iba modificando en cada operación, en diecinueve lugares distintos del programa. Bastaba con que una sola de esas modificaciones se perdiera —por una desconexión, por dos pestañas abiertas, por un error a mitad de camino— para que el número quedara mal para siempre, porque nadie lo volvía a calcular. Ahora el saldo se reconstruye igual que los lotes y el USDT: el último saldo que fijaste a mano, más el efecto de cada compra, venta, ajuste, transferencia y conversión posterior. Se recalcula en el mismo momento que todo lo demás, así que las tres cosas siempre concuerdan entre sí. Un descuento que no se aplicó, o que se aplicó dos veces, deja de importar: el número ya no depende de eso.'},
        {type:'improve', title:'Se retiró todo lo que existía para convivir con ese problema', desc:'La vigilancia automática de saldos, el aviso de saldo alterado y la corrección manual desde la pantalla de verificación fueron creados para detectar y arreglar desvíos. Al pasar el saldo a recalcularse solo dejaron de tener sentido: no hay nada que vigilar si el número se reconstruye en cada actualización. El programa quedó más liviano y con menos piezas que puedan fallar.'},
        {type:'improve', title:'La verificación quedó enfocada en lo que sí importa', desc:'Ya no compara saldos, porque no pueden estar desviados. Busca únicamente lo que sí es un dato mal cargado: registros repetidos y cifras imposibles, como un saldo negativo o un lote con más disponible del que se compró. También se quitó el botón de eliminar ajustes agregado en la versión anterior, porque ya se podían eliminar desde su propia fila.'}
    ]},
    {version:'5.9.0', date:'2026-08-15', headline:'🔓 Los lotes de arrastre ahora sí se pueden corregir.', changes:[
        {type:'fix', title:'Editar un lote de arrastre no guardaba nada', desc:'Desde que la definición de estos lotes pasó a guardarse en su propio campo, el editor cambiaba únicamente la copia que el motor calcula: al recalcularse, se reconstruía desde la definición original y el cambio desaparecía sin ningún aviso. Se veía exactamente como si los lotes de arrastre no se pudieran modificar. Lo mismo pasaba al eliminarlos: volvían a aparecer. Ahora el editor actualiza la definición, que es lo único que el recálculo respeta, así que corregir la cantidad de un lote de arrastre funciona y se mantiene.'},
        {type:'new', title:'Eliminar un ajuste desde su propia ventana', desc:'Para borrar un ajuste había que deslizar la fila hacia el costado, algo que no está indicado en ninguna parte y que en el teléfono es imposible de descubrir. Ahora la ventana de edición tiene su botón de eliminar, visible solo cuando estás editando uno existente.'}
    ]},
    {version:'5.8.3', date:'2026-08-15', headline:'🔧 El botón de verificar ya responde desde todos lados.', changes:[
        {type:'fix', title:'El botón no hacía nada al tocarlo', desc:'Los manejadores de la verificación habían quedado atados al panel del menú lateral, así que solo respondían a los toques dentro de ese panel. El botón que está dentro de Lotes de USDT y todos los de la pantalla de resultados —corregir saldos, cerrar, comparar con el servidor y recalcular los lotes de arrastre— quedaban fuera de su alcance: al tocarlos no pasaba absolutamente nada, y sin ningún aviso que lo explicara. Ahora escuchan en toda la pantalla. Además, si la verificación no pudiera iniciarse por algún motivo, ahora lo dice en vez de quedarse en silencio.'}
    ]},
    {version:'5.8.2', date:'2026-08-15', headline:'🔎 Ahora detecta registros repetidos aunque tengan otro identificador.', changes:[
        {type:'fix', title:'Un registro recreado tras un fallo pasaba desapercibido', desc:'La verificación buscaba identificadores repetidos, y eso deja fuera justo el caso más común: cuando la app falla a mitad de una operación, el registro se vuelve a crear con un identificador nuevo. Queda entonces un gemelo idéntico en todo lo que importa —tipo, monto, cuenta y categoría— con pocos minutos de diferencia, y a los cálculos les entra dos veces. Ahora se los busca comparando el contenido dentro de una ventana de diez minutos: lo bastante amplia para atrapar un reintento fallido y lo bastante angosta para no confundir dos gastos reales parecidos. Dos gastos iguales en días distintos, o con la misma cifra pero otra categoría, no se marcan.'},
        {type:'new', title:'Los repetidos se ven y se borran desde la misma pantalla', desc:'Antes la verificación decía cuántos había, pero no cuáles, así que desde el teléfono no había forma de arreglarlo. Ahora cada uno aparece con su descripción, su fecha y el motivo por el que se lo marcó, con un botón para borrarlo. El borrado usa el mismo camino que el botón de la lista, así que revierte igual su efecto sobre saldos y lotes, y al terminar vuelve a verificar para mostrar cómo quedó todo.'},
        {type:'new', title:'Avisa de estados que no pueden darse', desc:'Un saldo negativo, un saldo USDT negativo o un lote con más disponible del que se compró siempre significan que algo se contó de más, pero hasta ahora la app no lo mencionaba en ningún lado. Ahora aparecen listados en la verificación, junto con la aclaración de que suelen resolverse borrando el registro repetido que los causó.'}
    ]},
