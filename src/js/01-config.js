...
const CHANGELOG = [
    ...
    {version:'4.9.3', date:'2026-07-19', headline:'🔌 Conexión con Firestore a prueba de redes rotas y colas atascadas.', changes:[
        {type:'fix', title:'"Modo local activo" eterno y ping que fallaba incluso con WiFi', desc:'Dos causas reales, dos fixes. (1) La app usa persistencia: la cola de escrituras del SDK vive en IndexedDB y SOBREVIVE al cerrar la app — el write grande que quedó colgado en un intento anterior bloqueaba en orden todo lo nuevo (por eso el ping moría a los 15s sesión tras sesión). Ahora el doctor de conexión lo detecta, prueba un reinicio suave del canal, y si no alcanza ofrece "🧹 Reiniciar conexión y recargar", que limpia SOLO esa cola interna: tus operaciones no se tocan (viven en el respaldo local y se restauran al volver). (2) Se activó experimentalAutoDetectLongPolling: en redes o proxies que rompen el canal de streaming de Firestore, el SDK ahora degrada solo a long-polling en vez de colgarse.'},
        {type:'fix', title:'La pantalla de error mostraba textos superpuestos', desc:'El sello de versión había desplazado al subtítulo del overlay (selector posicional): en el estado de error quedaba visible "No cierres la app…" junto al mensaje de fallo y el sello desaparecía. Ahora cada elemento tiene su id y el error se muestra limpio, con la versión siempre visible.'}
    ]},
