 ...
 */
const CHANGELOG = [
    {version:'5.4.3', date:'2026-08-07', headline:'📊 Gráfica de ganancia rehecha y arreglada la fila de computadora.', changes:[
        {type:'fix', title:'En computadora la fecha caía a una segunda línea y quedaba cortada', desc:'Al ocultar los puntos separadores de la fila en una versión anterior se usó una regla más específica que la de computadora, y esas reglas ganan aunque estén fuera de su pantalla. Resultado: en pantalla grande los separadores seguían contando como columnas de la tabla, sobraban dos, y la fecha junto con los botones de editar y borrar caían a una fila de abajo que quedaba recortada. Ahora los separadores se ocultan de verdad y la separación la da el espaciado de la fila, así que el conflicto desaparece en las dos pantallas.'},
        {type:'improve', title:'La gráfica de ganancia ahora muestra barras por día', desc:'Mostraba una línea, y una línea sugiere que entre un día y el siguiente hubo una transición gradual, cosa que no ocurre: la ganancia diaria son valores sueltos. Ahora hay una barra por día, verde o roja según cómo cerró, con el día de hoy destacado y los anteriores atenuados, más una línea punteada en el cero cuando hubo pérdidas, para saber de un vistazo cuántos días cerraron en cada lado. Además se dibujaba siempre a un tamaño fijo aunque el espacio reservado es bastante mayor en pantalla grande, y por eso se veía chica y perdida: ahora se adapta al espacio disponible.'},
        {type:'improve', title:'El campo de cuenta avisa que es obligatorio', desc:'Se veía igual que un campo opcional y el aviso llegaba recién al tocar el botón de comprar. Ahora el rótulo lleva una marca y el campo queda en ámbar mientras no elijas la cuenta. Ámbar y no rojo a propósito: todavía no es un error, es algo que falta.'},
        {type:'improve', title:'Ya se ve dónde estás al usar el teclado', desc:'No había ninguna regla de foco en toda la hoja de estilos: al recorrer el formulario con el tabulador no había forma de saber qué campo estaba activo. Ahora aparece un aro de color, y solo con teclado: al tocar con el dedo no molesta.'},
        {type:'fix', title:'El botón de comprar volvía a mostrar un emoji', desc:'El texto de ese botón se reescribe desde el programa cada vez que cambiás el monto, y esa reescritura pisaba el ícono puesto en la página. Ahora el ícono viaja junto con el texto.'}
    ]},
    {version:'5.4.2', date:'2026-08-07', headline:'🖥️ Arreglada la vista de computadora y ampliado el panel de nueva operación.', changes:[
        {type:'fix', title:'En computadora la fecha se cortaba y los botones quedaban en el medio de la fila', desc:'En pantalla grande la lista se arma como una tabla de columnas fijas, y esa tabla estaba definida para ocho: monto, ganancia, tasa, USDT, comisión, banco, fecha y acciones. Al quitar la comisión de la fila en la versión anterior quedaron siete elementos para ocho columnas, así que todo se corrió un lugar: la fecha terminó en la columna de acciones, que mide apenas lo necesario para dos botones, y por eso se veía cortada como "07/08 0"; los botones de editar y borrar, a su vez, aparecían flotando sobre el medio de la fila. Ahora la tabla tiene siete columnas y la fecha tiene ancho propio, así que no puede recortarse.'},
        {type:'improve', title:'El panel de nueva operación ya no está apretado', desc:'En computadora ocupaba una columna de 320 píxeles y para entrar ahí se encogía todo: los rótulos quedaban en nueve píxeles, los campos en catorce y los botones de subir y bajar la tasa en treinta y dos. Es la acción principal de la app y se veía como un panel secundario. La columna pasó a 372 píxeles y todo volvió a tamaños cómodos. El espacio sale de la lista, que sigue teniendo aire de sobra.'},
        {type:'improve', title:'Últimos emojis reemplazados', desc:'El logo, la campana de novedades, el estado de lista vacía y el botón de usar categoría existente pasaron a íconos dibujados. Quedan seis emojis en lugares donde el navegador no permite otra cosa: las opciones de las listas desplegables y los textos de los campos de búsqueda.'},
        {type:'improve', title:'Botones de editar y borrar más fáciles de acertar', desc:'En computadora medían veinte píxeles de alto y están pegados uno al otro. Subieron a veintiséis y se separaron un poco más, sin agrandar el dibujo.'}
    ]},
    ...
    {version:'5.4.0', date:'2026-08-06', headline:'✨ Interfaz más legible: números alineados, íconos propios y colores centralizados.', changes:[
        ...
        {type:'fix', title:'La etiqueta cortada del encabezado', desc:'El tercer indicador de Ganancia Hoy mostraba "vs prom. se…" porque el texto no entraba en el ancho de la columna. Un rótulo cortado no informa nada, así que ahora dice "vs. semana", que significa lo mismo y entra completo.'},
        {type:'improve', title:'Íconos propios en lugar de emojis', desc:'Los rótulos de sección, el menú lateral y el resumen mensual usaban emojis. Cada sistema los dibuja a su manera, por eso se veían distintos en iPhone, Android y Windows, y traían su propio color, que chocaba con la paleta. Se reemplazaron por treinta y seis íconos dibujados con trazos, que se ven idénticos en todos lados y toman el color del texto. Quedan emojis solo donde no se pueden usar íconos por limitaciones del navegador: las listas desplegables, los textos de ayuda y los campos de búsqueda.'},
        {type:'improve', title:'Colores centralizados', desc:'La hoja de estilos tenía ciento dieciséis variables de diseño definidas pero casi sin usar, y en paralelo más de quinientos colores escritos a mano. Por eso un mismo gris variaba entre pantallas. Se reemplazaron trescientos cuarenta por su variable correspondiente, verificando que el color resultante es exactamente el mismo que antes en cada caso. A partir de ahora, ajustar un tono se hace en un solo lugar.'}
    ]},
