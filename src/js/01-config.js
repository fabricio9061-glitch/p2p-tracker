...
const CHANGELOG = [
    {version:'4.8.2', date:'2026-07-12', headline:'🛡️ Auditoría de estabilidad: 8 fixes de sync, datos y seguridad.', changes:[
        ...
        {type:'fix', title:'escHtml no escapaba comillas (inyección de atributos)', desc:'escHtml se usa dentro de atributos data-tag="...", y un tag o descripción con comillas rompía el atributo permitiendo inyectar handlers (self-XSS). FIX: escHtml ahora escapa & < > " \' y además es más rápido (sin crear un div por llamada, relevante en loops de render).'},
        {type:'fix', title:'El payload guard (>850 KB) dejaba pasar el write que lo disparó', desc:'El guard activaba safe mode pero solo frenaba writes FUTUROS; el payload crítico en curso igual se escribía una vez. FIX: se aborta también el write actual con rollback de la versión optimista.'},
        {type:'fix', title:'Cerrar sesión no limpiaba los datos en RAM', desc:'AppState.datos conservaba los datos del usuario saliente; otra persona iniciando sesión en el mismo dispositivo podía verlos renderizados por el watchdog de 3s antes del primer snapshot, y la sync queue vieja contaminaba su merge. FIX: reset de datos, flags y queue en el logout.'},
