...
const CHANGELOG = [
    ...
    {version:'5.4.6', date:'2026-08-08', headline:'📄 La lista dice cuáles estás viendo y la pantalla dejó de correrse sola.', changes:[
        {type:'fix', title:'La pantalla se iba bajando al tocar varias veces un botón', desc:'Pasaba con los botones de subir y bajar la tasa y con el de página siguiente, y tenía dos causas. La primera: al tocar dos veces seguidas, el navegador del teléfono espera unos milisegundos para decidir si fue un doble toque para ampliar, y durante esa espera puede tomar el movimiento del dedo como un desplazamiento; ahora se le indica que esos botones solo aceptan toques simples y esa espera desaparece. La segunda: cambiar de página reemplaza todas las filas, y como no miden exactamente lo mismo el alto total varía unos píxeles y el navegador reacomoda el desplazamiento, corrimiento que se acumulaba al tocar seguido; ahora la posición se guarda antes de redibujar y se restaura después.'},
        {type:'improve', title:'La paginación dice el rango, no solo el número de página', desc:'Antes mostraba "1 / 29", que dice en qué página estás pero no cuáles operaciones estás viendo ni cuántas hay. Ahora dice "1–25 de 286". Además la lista pasó de diez a veinticinco por página: con doscientas ochenta y seis eran veintinueve páginas y llegar a una del medio pedía una docena de toques; ahora son doce. Los botones crecieron a cuarenta y cuatro píxeles de alto, la medida mínima recomendada para el dedo, y se separaron a los extremos con el rango centrado.'},
        {type:'fix', title:'Los títulos de sección quedaban desalineados con su ícono', desc:'El ícono se apoyaba en la línea base del texto, lo que en un título grande lo dejaba visiblemente más bajo que la palabra. Ahora se centra con la altura real del texto, sin importar el tamaño.'}
    ]},
