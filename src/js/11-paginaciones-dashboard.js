...
const pagOp=crearPaginacion({
    ...
    renderFn:(ini,fin,total,allOps)=>{
        ...
        ops.forEach(op=>{
            ...
            const gn=op.ganancia||0;
            /* v5.4.0 — Un resultado exactamente en cero se muestra en gris. El verde
               queda reservado para ganancia real: en una lista donde la mitad de las
               filas son compras sin comisión, pintarlas de verde diluye la señal. */
            const gc=Math.abs(gn)<0.005?'#94a3b8':colorGan(gn);
            const gt=fmtGan(gn);
